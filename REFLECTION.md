...
# Assignment 14 Reflection: Peer Review, Onboarding, and Open-Source Collaboration

StudySync was originally developed as an academic software engineering project, but Assignment 14 required the repository to be prepared as if outside contributors could join the project. This changed the way I looked at the repository. Instead of only asking whether the project works for me, I had to ask whether another student could understand the purpose of the system, set it up, run the tests, choose a task, and submit a pull request without needing private explanations from me.

One of the main improvements I made was strengthening the documentation. I added a `CONTRIBUTING.md` file that explains the prerequisites, installation steps, testing commands, coding standards, issue workflow, branch workflow, and pull request expectations. This is important because new contributors often struggle when a project does not clearly explain how to get started. I also updated the `README.md` with a contributor-focused Getting Started section and a Features for Contribution table. These additions make the repository easier to review because contributors can quickly see which tasks are beginner-friendly and which tasks are larger future improvements.

I also added a `ROADMAP.md` file to show where the project can go next. The roadmap includes short-term documentation and testing improvements, medium-term user experience and study group features, and long-term improvements such as Redis caching, Docker support, OAuth login, and monitoring. This helped make the project feel more open to future work rather than closed after the assignment. I also added an MIT `LICENSE` so that the project has a clear open-source usage policy. Without a license, contributors may be unsure whether they are allowed to reuse, fork, or improve the code.

Another improvement was preparing contributor issues. Since GitHub labels and issues must be created directly on GitHub, I added `ISSUE_LABELS.md` as evidence and a guide. It lists more than five `good-first-issue` tasks and more than three `feature-request` tasks. The beginner issues focus on simple improvements such as validation, JavaDoc comments, README formatting, API notes, and extra unit tests. The feature-request issues focus on larger enhancements such as dark mode, Google Calendar integration, and Redis caching.

A challenge in onboarding contributors is that people may have different development environments. Some classmates may use IntelliJ, others may use VS Code, and some may not have Java, Maven, or Node.js installed correctly. To reduce this problem, I made the setup instructions clear and included the exact Maven commands for running tests. Another challenge is making sure contributors understand the existing structure before making changes. To address this, the README includes links to the main packages, tests, documentation, and CI/CD workflow.

I also learned that peer review is not only about finding mistakes. It is also about making the project easier for others to understand. Stars and forks are useful signals, but the real value comes from feedback, questions, and pull requests. If a classmate struggles to set up the project, that shows a documentation problem. If someone asks where to start, that shows the issue labels or roadmap may need improvement. Overall, this assignment helped me understand that open-source collaboration depends on clear communication, good documentation, small beginner-friendly tasks, and a respectful review process.
